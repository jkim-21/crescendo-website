      ...
      <div className={`${styles.flexCenter} flex sm:mb-20`}>
        {stats.map((stat) => (
          <div key={stat.id} className={`flex flex-col justify-center items-center mt-10 px-[3rem] ${!stat.lastStat ? 'border-r border-[#b2b2b2]' : ''}`}>
            ...
            <h4 className='font-semibold xs:text-[40px] text-[30px] xs:leading-[53px] leading-[43px] black-color'>
              {stat.value}
            </h4>
            <p className='font-normal text-center xs:text-[20px] text-[15px] xs:leading-[26px] leading-[21px] text-gradient-dark uppercase'>
              {stat.title}
            </p>
          </div>
        ))}
